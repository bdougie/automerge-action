...
async function update(context, dir, url, pullRequest) {
  ...
  const actions = [automerge, autorebase, "dependencies", "security"];

  let action = null;
  logger.info(pullRequest.labels);
  logger.info(`ACTION: ${actions} from actions`);

  logger.info(`ACTION: ${action} for action`);
  ...
  if (action === automerge || action === "dependencies" || action === "security" || action === null) {
    return await merge(octokit, pullRequest);
  } else if (action === autorebase || action === "dependencies" || action === "security" || action === null) {
